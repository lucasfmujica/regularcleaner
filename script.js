...
$(document).ready(function() {
  ...
  var displayValue = document.querySelector('[fs-rangeslider-element="display-value"]');
  ...
  function updateExtraTime() {
    ...
    var totalTime = Math.ceil(Number(bedroomsInput.value) * 0.75 + Number(bathroomsInput.value) * 0.75 + extraTime);
    ...
    hoursRecommended.textContent = (recommendedHours < 8 ? recommendedHours : "8+") + ' hours';
  ...
  decreaseBedroomsButton.addEventListener('click', function(e) {
    ...
    if (currentValue > 1) {
      ...
      bedroomsLabel.textContent = bedroomsInput.value + " Bedroom" + (bedroomsInput.value > 1 ? "s" : "");
    }
    updateExtraTime();
  });
  ...
  decreaseBathroomsButton.addEventListener('click', function(e) {
    ...
    if (currentValue > 1) {
      ...
      bathroomsLabel.textContent = bathroomsInput.value + " Bathroom" + (bathroomsInput.value > 1 ? "s" : "");
    }
    updateExtraTime();
  });
  ...
  extraServices.forEach(function(service) {
    ...
    if (input.value === 'false') {
      ...
      summaryExtra.forEach(function(summary) {
        var item = summary.querySelector('.calculator_summary-extra-item[data-service="' + service.id + '"]');
        if (item) {
      ...
      summaryExtra.forEach(function(summary) {
        ...
        itemText.classList.add('text-size-small', 'text-weight-bold', 'text-color-light-grey');
        itemText.textContent = "+ " + service.id.charAt(0).toUpperCase() + service.id.slice(1);
...
$(document).ready(function() {
    ...
    function updateDateSelector() {
        ...
        const startDay = document.documentElement.clientWidth <= 479 ? 2 : 3; // Comienza dos días antes en móviles

        monthDisplay.textContent = monthNames[currentDate.getMonth()] + ' ' + currentDate.getFullYear();

        for (let i = 0; i < daysToShow; i++) {
            const date = new Date(currentDate.getTime());
            date.setDate(currentDate.getDate() - startDay + i); // Esto hace que la fecha actual se muestre en el tercer elemento en móviles
    ...
    const currentDayElement = document.getElementById('day' + (document.documentElement.clientWidth <= 479 ? 3 : 4));
...
$(document).ready(function() {
    ...
    function updateOftenDisplay() {
        ...
        const selectedOption = howOftenSelect.val();
